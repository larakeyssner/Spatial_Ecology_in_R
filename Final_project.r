...
#Assumption: 
  #Initially assumption is that bird observations would be more frequent in urbanized areas
...
#The aim was to choose a study are with around 50% natural areas and 50% urban areas 
#Why the City park Eilenriede in Hanover:
  #Because it is one of the biggest forest included into a city in Europe 
...
#About the data:
  ...
  #Species analyzed (Scientific name): Aves (898 occurrences)
...
#Adding the before installed packages to the library so they can be used 
library(viridis) #Color palette easier to read by those with colorblindness
...
library(ggplot2) #for making high quality plots with the possiblity of adding muliple features
...
#Using \t to have taps as speration between values 
...
#Making the coordinates numeric to remove any kind of signs (in this case the quotation marks) 
occ$decimalLatitude  <- as.numeric(occ$decimalLatitude)
occ$decimalLongitude <- as.numeric(occ$decimalLongitude)

#To check if all values numeric now 
occ[[98]] #Latitude 
occ[[99]] #Longitude
...
#Only for the column of the coordinates --> so now only data that have both coordinates
...
#Visualization of bird observation density in the study area
...
#Smoothing point location to develop density map
dmap_bird <- density(occ_points)


#Defining the color palette for graphs using Viridis, which is easily readable for colorblind individuals
#The number 100 specifies that the palette will contain 100 distinct color levels
cl <- viridis(100)
...
#Visualizing bird observation density: first as raw density, then normalized (0–1) for easier interpretation
#The normalized scale is more inuative to understand and read
...
#To have a look at the information regarding the two rasters to chekc if everthing worked smoothly 
rastB4
rastB8
...
#Calculation of the Normalized difference vegetation index (NDVI) --> ranging from -1 to 1 
...
#Subset bird densiyt values by the landcover category (urban pixel and natural pixels)
#Two indendent samples that can be compared 
...
#Interpretation: 
  #W value (rank of the sums): large W usually with large samples and if ranks are strongly seperated 
...
#Extract NDVI values at bird locations which allows spatial overlay points and raster layer
...
#Adding the bird observations points into the graph of the land-cover cluster 
...
#Next step: to analysis what kind of land-cover is associated with more or less bird observations?
...
#To create a panel with two plot in one row 
par(mfrow= c(1,2))
...
#Interpretation:
  ...
  #Recognizable hot-spot where urban and forest analysis overlaps – further analysis is needed to understand the factors driving high bird activity in this mixed landscape
...
#First step: visual analysis of the area loading the true color satellite image  

#Loading the true color satellite image as a raster
...
summary(truecolor) 
#The maximum is almost 1 --> setting the scale until 1 for the vizualisation 
...
#Interpretation:
  ...
  #However, the RGB true-color imagery clearly shows that the area with a high concentration of bird observations contains a lake
  ...
  #The combination of open water, vegetation, and recreational park infrastructure likely provides favorable habitat conditions for birds,
...
#The initial assumption that bird observations would be more frequent in highly urbanized areas was not supported by the analysis
#Instead, a higher number of observations were recorded in vegetated areas
#This pattern is likely due to the presence of trees, shrubs, and other vegetation between buildings, providing natural habitat within the ciyt frame 
#Even within urban areas, these vegetated patches attract birds and are also accessible to humans, increasing the likelihood of observations being recorded in natural areas 
#NDVI-based analysis confirmed that bird observations are strongly associated with areas of higher vegetation density, even when these areas are interspersed within urban structures
#This indicates that bird density is influenced more by habitat quality (vegetation cover) than by urbanization alone
#Overall, the spatial distribution of bird observations in Eilenriede City Park demonstrates that natural features within urban environments play a critical role in supporting wildlife and influencing observation patterns 
#These findings directly address the research question, showing that bird densities differ significantly between urban and natural land-cover types and that vegetation cover (NDVI) is a key explanatory factor
